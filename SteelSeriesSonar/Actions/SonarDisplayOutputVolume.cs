...
{
    [PluginActionId("com.mrayianalex.sonar.displayoutputdevice")]
    class SonarDisplayOutputAction : KeypadBase
    {
        ...
        public SonarDisplayOutputAction(SDConnection connection, InitialPayload payload) : base(connection, payload)
        {
        ...
        public override void ReceivedGlobalSettings(ReceivedGlobalSettingsPayload payload)
        {
            ...
            {
                Logger.Instance.LogMessage(TracingLevel.WARN, "SonarDisplayOutputAction : Global Settings do not exist!");
                SetGlobalSettings();
            }
            ...
            else
            {
                ...
                if (settings.RenderHorizontal)
                {
                    ...
                    graphics.FillRectangle(WhiteBrush, 1, 59, (int)Math.Ceiling(144 * ReturnVolume(global, settings.OutputDevice)), 29);
                }
                else if (settings.RenderVertical)
                {
                    ...
                    // graphics.DrawLine(WhitePen, new PointF(59, 1), new PointF(144, 1));
                    // draw a rectangle représente volume level
                    graphics.FillRectangle(WhiteBrush, 59, 144 - (int)Math.Ceiling(144 * ReturnVolume(global, settings.OutputDevice)), 29, (int)Math.Ceiling(144 * ReturnVolume(global, settings.OutputDevice)));
                }
                else if (settings.RenderCircle)
                {
                    ...
                    // draw a rectangle représente volume level
                    int radius = (int)Math.Ceiling(70 * ReturnVolume(global, settings.OutputDevice));
